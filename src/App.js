...
function App() {
  ...
  const syncSingleMatch = async (match, selectedTeam = "ALL") => {
    try {
      ...
      // This is match-level filtering — only players in statsMap played
      // For team filtering, build a set per IPL team
      ...
      // by checking which innings they appeared in
      ...
      scorecardData.scorecard.forEach(inning => {
        ...
        // Bowlers belong to the OPPOSITE team of this inning's batting team
        inning.bowling?.forEach(bw => {
          const name = normalizeName(bw.bowler?.name);
          if (!name) return;
          if (isTeamA) playerToIPLTeam[name] = teamBFull; // bowling against teamA = teamB
          else if (isTeamB) playerToIPLTeam[name] = teamAFull; // bowling against teamB = teamA
        });
      });
